<template>
  <div class="h-100" style="position: relative">
      <div class="px-8 pt-4 d-flex align-center" style="
              position: fixed;
              top: 50px;
              right: 0px;
              z-index: 1000;
          ...
          ">
          <v-text-field variant="underlined" v-model="search" placeholder="search room" />
          <v-btn class="ml-3" @click="dialog=true">Create Room</v-btn>
      </div>
      <div class="mt-14 pt-1">
          <div v-if="data.length != 0">
              <v-card v-for="i in data" class="my-4" @click="goToRoomView(i.roomId)" :key="i">
  ...
  <v-dialog
    ...
    >
      ...
      <v-card>
       ...
       <v-btn style="position: fixed; bottom: 10px; right: 10px; z-index: 300" @click="createRoom"
       >
        Create Room
      </v-btn>
      </v-card>
