<template>
  <v-app>
    ...
    <v-bottom-navigation :elevation="6" grow :active="true" :model-value="route.name" @update:model-value="(e)=>goPage(e)">
...
const redirectWithButtonClick = localStorageRedirect != null?JSON.parse(localStorageRedirect) : false 
...
(async()=>{
  ...
  if(!redirectWithButtonClick){
    ...
    if(roomId != null){
      ...
      const response = await fetch(`http://localhost:3001/getRoomId`).then((e) => e.json())
